...
BatchEnv::BatchEnv(const std::vector<EnvConfig>& configs) {
  if (configs.empty()) {
    throw std::invalid_argument("configs must not be empty");
  }
  envs_.reserve(configs.size());
...
void BatchEnv::reset_all(uint64_t seed, float* obs_out) {
  ...
  const int parallel_threads = std::min(45, n);
#pragma omp parallel for schedule(static) if (n >= 2) num_threads(parallel_threads)
#endif
  for (int i = 0; i < n; ++i) {
