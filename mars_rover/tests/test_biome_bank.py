...
def test_chain_biomes_off_by_default_keeps_single_unbounded_zone() -> None:
    # EnvConfig defaults to chain_biomes=False; env.yaml (the training config)
    # opts in explicitly, so this constructs the config default directly.
...
def test_chain_biomes_builds_a_bounded_concatenation() -> None:
    ...
    for seed in range(12):
        ...
        debug = env.debug_info()
        # The chain always leads with a short lead-in before the first boundary,
        # and each zone is drawn from the configured [min, max) segment length -
        # never the old single-zone [-1e6, 1e6] span.
    ...
    env.close()
    # A different seed must be able to produce a different course: this is the
    # whole point of the chain (a fresh trial should not replay the same track).
...
def test_sampling_modes_select_exact_expected_biome_sets() -> None:
    ...
    for mode, expected in expected_by_mode.items():
        # chain_biomes=False: this test checks split ELIGIBILITY, not the
        # chain-assembly behaviour covered separately above.
