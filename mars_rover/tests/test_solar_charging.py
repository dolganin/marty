...
def _solar_env(seed: int, biome_id: int):
    config = EnvConfig()
    # Keep this built-in regression independent of an installed LLM train bank.
...
def test_solar_cycle_locks_rover_and_charges_fastest_on_ice() -> None:
    # These deterministic seeds start in Ice and Sand respectively.
    ...
    # Stowing also locks propulsion. The engine remains off until E is used.
    ice_step(1024)
    stow_x = ice["x"]
    for _ in range(120):
        ice = ice_step(1)
