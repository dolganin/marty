...
// Useful RPM in the next gear at light throttle. Throttle and grade raise the
// recommendation, so a full-throttle acceleration uses more of the engine's
// rev range while relaxed driving can shift earlier.
...
PhysicsStepStats PhysicsEngine::step(const RoverRig& rig, const Terrain& terrain, RoverState& state,
  ...
  // Lidar and wheel scanners are contact instruments in this benchmark. Once
  // committed to flight they go silent; an active pulse cannot be carried over
  // the lip and a new pulse cannot be fired until ground contact returns.
  if (was_airborne) {
  ...
  // Public macro `gas + shift-up` is a sequential upshift request while the rover
  // continues to accelerate.  The mechanical speed/RPM guard below still decides
  // whether a gear can engage; this only avoids demanding six frame-perfect manual
  // clutch pulses before a ballistic ledge can ever be explored.
  const bool powered_upshift_request =
      control.shift_up && std::abs(control.throttle) > 0.5f && !clutch_pedal_down;
  // A held clutch+shift command is a physical sequential shift request: it waits for
  // each synchroniser cooldown, then requests the next legal gear.  This preserves
  // the low-level clutch/gear dynamics, while making a deliberate "prepare for the
  // ledge" action learnable from sparse reward.  Previously six distinct press/release
  // pulses at exact 45-step intervals were required to reach seventh gear; a random
  // exploration policy therefore almost never observed a successful ballistic takeoff.
  // Releasing the button still cancels the request after the normal 60-step buffer.
  ...
  // Body pitch is a stable proxy for sustained drivetrain load. Instant tyre
  // slip is deliberately excluded here: it oscillates during clutch operation
  // and made the recommended shift RPM jump between adjacent frames.
  ...
  if (state.gear_index >= 0 && state.gear_index < kGearCount - 1) {
    ...
    // RPM and road speed are coupled through kGearMaxSpeeds everywhere else
    // in the drivetrain. Derive both thresholds from the same relation instead
    // of mixing those speeds with the nominal ratio table.
  ...
  // Integrate the crankshaft instead of snapping RPM to a target every frame.
  // motor_torque / inertia gives angular acceleration; 60 / (2*pi) converts it
  // from rad/s^2 to RPM/s. With the clutch open the engine free-revs against
  // internal drag. With it closed, a finite-rate clutch synchronizes the
  // crankshaft to road speed, allowing visible slip during launch and shifts.
  ...
  if (!in_neutral && state.clutch_engagement > 0.001f) {
    ...
    // Most combustion torque is transmitted to the wheels with the clutch
    // closed; only a small fraction accelerates the rotating assembly itself.
  ...
  state.engine_cold_locked =
      ...
      state.engine_temperature < config_.cold_start_temperature;
  state.engine_stalled = !state.engine_running;
  // Every generated biome may alter gravity and wind, independently of its
  // visual category. This keeps visual appearance from revealing mechanics.
  ...
  for (int i = 0; i < state.wheel_count; ++i) {
    ...
    // A height query directly below the hub cannot see the leading face of a
    // bump until the hub is already inside it. Probe the leading part of the
    // tyre as well, using the circle profile for its vertical extent. This is
    // the inexpensive height-field equivalent of a circle/terrain contact.
    ...
    const float tyre_half_height = wheel.radius * 0.693974f;  // sqrt(1 - 0.72^2)
    ...
    // Keep a small persistent contact manifold. Without this slop the
    // positional solver separates the tyre exactly to zero, so propulsion is
    // present only every other frame (fall -> correct -> no contact -> fall).
    ...
    if (terrain_sample.solid && penetration > -kContactSlop) {
      ...
      const float speed_fade = driven_speed > 0.0f
                                   ? clamp(1.0f - driven_speed / std::max(0.1f, gear_max_speed),
                                           0.0f, 1.0f)
                                   : 1.0f;
      // One motor drives every wheel. Split its output across the complete
      // drivetrain instead of accidentally granting full engine torque to
      // every wheel independently.
      ...
      MechanicContext ctx{};
      // Positional contact correction leaves almost zero penetration on the
      // next frame. Do not let that erase all available tyre grip: a grounded
      // wheel still carries its share of the rover's static weight.
      ...
      // First gear is a crawler gear. At very low speed its reduction must be
      // able to break static equilibrium even when the sampled surface has a
      // low dynamic-friction modifier.
    ...
    if (wheel.in_contact) {
      ...
      // A wheel rolling without slipping in the +x direction spins clockwise in this
      // x-right/y-up/angle-CCW frame, i.e. opposite sign to its linear speed.
  ...
  if (!grounded_now) {
    // Do not classify the short spawn-settling transient as a committed
    // ballistic phase. Flight starts only after contact has existed and is lost.
