...
void Renderer::render_rgb(const Env& env, uint8_t* rgb, int width, int height) {
  ...
  if (body_style.particles.ambient_particles > 0) {
    // A light, persistent crosswind of fine dust. This is screen-space on
    // purpose: the storm remains visible even while the rover is stationary.
  ...
  for (int i = 0; i < state.wheel_count; ++i) {
    ...
    draw_line(rgb, width, height, static_cast<int>(wp.x), static_cast<int>(wp.y), sx, sy,
              {230, 230, 230});
    draw_line(rgb, width, height,
              static_cast<int>(wp.x + std::cos(spoke_angle + 1.5707963f) * radius * 0.75f),
              static_cast<int>(wp.y - std::sin(spoke_angle + 1.5707963f) * radius * 0.75f),
              static_cast<int>(wp.x + std::cos(spoke_angle - 1.5707963f) * radius * 0.75f),
              static_cast<int>(wp.y - std::sin(spoke_angle - 1.5707963f) * radius * 0.75f),
              {190, 190, 190});
  ...
  // Tint submerged rover parts and particles. The water surface is horizontal
  // inside a basin while the bottom follows the carved terrain.
  for (int x = 0; x < width; ++x) {
    const float wx = camera_x + static_cast<float>(x) / ppm;
    const auto& zone = env.mechanic_at(wx);
