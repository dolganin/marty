...
def _print_biome_catalog() -> None:
    ...
    for biome in biome_catalog():
        p = biome["parameters"]
        # An editable Python install may briefly be newer than the compiled
        # extension (especially on Windows while a running player locks the
        # .pyd). Keep startup usable and clearly mark legacy visual metadata.
        v = biome.get("visuals", {
            "ground_rgb": (42, 35, 30),
            "particle_rate": 0.0,
            "ambient_particles": 0,
