...
def load_rover_rig(path: str | Path) -> RoverRig:
    ...
    if body_data:
        body = BodyRigConfig()
        body.sprite = str(body_data.get("sprite", body.sprite))
    ...
    for wheel_data in data.get("wheels", []):
        ...
        wheel.name = str(wheel_data.get("name", wheel.name))
        wheel.sprite = str(wheel_data.get("sprite", wheel.sprite))
    ...
    parts = []
    for part_data in data.get("visual_parts", []):
        part = VisualPartRigConfig()
        part.name = str(part_data.get("name", part.name))
        part.sprite = str(part_data.get("sprite", part.sprite))
        part.parent = str(part_data.get("parent", part.parent))
        part.local_position = _vec2(part_data.get("local_position"), part.local_position)
        part.local_rotation = float(part_data.get("local_rotation", part.local_rotation))
        part.scale = _vec2(part_data.get("scale"), part.scale)
        part.collision = _collision(part_data.get("collision"))
        parts.append(part)
    rig.visual_parts = parts
