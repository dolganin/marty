...
def build_context(
    ...
    *,
    ...
    env.close()
    # The trial context only changes at episode boundaries, so within the probe episode it
    # is a constant vector — which is what makes swapping both channels cleanly possible.
    ...
    for label, model_path in args.models:
        ...
        for trial in range(args.trials):
            ...
            for index, item in enumerate(selected):
                ...
                # Swap BOTH carriers of trial information together: attention state and
                # the explicit context summary. The question is whether the policy uses
                # what earlier episodes taught it, by whichever route it is available.
