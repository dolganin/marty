...
def distill_from_ppo_teacher(
    ...
    *,
    ...
    update = 0
    model.train()
    # The environments and the recurrent state persist across updates. Resetting them
    # every chunk would train the student only on an empty-to-chunk_length memory, while
    # at 500 steps into a real episode the memory holds `memory_len` entries — a regime
    # the student would then never have seen. That mismatch, not the loss shape, is what
    # made warm-entered episodes collapse.
    ...
    while completed < transitions:
        ...
        with torch.no_grad():
            for _ in range(chunk_length):
                ...
                for env_index in np.flatnonzero(dones):
                    ...
                    if trial_start:
                        # Memory survives episode boundaries and dies at trial boundaries.
                        # The student's state only advances when the chunk is replayed
                        # below, so the reset is deferred until after that replay.
        ...
        # Truncated BPTT: gradients stop at the chunk boundary, but the memory contents
        # carry over, so later chunks are trained on a realistically full memory.
