...
build the native extension, run the test suite and start the manual debugger on Windows and Linux.
...
All commands below are run from the repository root. In a container running as root, omit
...
3. To generate LLM biomes, create the ignored local configuration:
   ...
   ```bash
   cp environment/python/mars_rover_env/configs/biome_generator.yaml \
      environment/python/mars_rover_env/configs/biome_generator.local.yaml
   chmod 600 environment/python/mars_rover_env/configs/biome_generator.local.yaml
   ```
...
Copy `environment/python/mars_rover_env/configs/biome_generator.yaml` to
...
both `environment/cpp/src/*.cpp` and `environment/cpp/bindings/pybind_module.cpp` work with a Debug build. Re-run setup after
...
The standalone `environment/cpp/CMakeLists.txt` is also portable and useful for IDE indexing, but the supported
