...
# Resolve configs through the installed package, not the repository layout.
PACKAGE_CONFIGS = Path(mars_rover_env.__file__).resolve().parent / "configs"
...
def test_hidden_mechanics_seed_is_frozen_within_trial() -> None:
    ...
    env = MarsRoverEnv(config_path=PACKAGE_CONFIGS / "play.yaml", chain_biomes=False)
...
def test_chain_biomes_builds_a_bounded_concatenation() -> None:
    ...
    # The opening zone is drawn from the anchor pool so runs stay comparable,
    # so its mechanic may well be the same every time; the chain varies later.
    assert seen_first_mechanics
