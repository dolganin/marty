...
def test_overheat_shutdown_and_cooldown_restart() -> None:
    ...
    config.physics.engine_thermal_mass = 1.0
    config.physics.engine_heat_per_fuel = 730.0
    config.physics.engine_idle_heat = 2.2
    config.physics.engine_cooling_conductance = 0.08
    config.physics.engine_cooling_airflow = 0.012
    _, obs, step = _batch(config, seed=0)

    step(512)  # v13 does not start the engine for you.
    for _ in range(3000):
        debug = step(1 | 8)                                  
    ...
    assert not debug["engine_running"]
    # Restarting needs neutral or the clutch pedal, not the key alone.
    debug = step(512 | 8)
...
def test_cold_engine_cannot_start_in_a_colder_martian_biome() -> None:
    ...
    config.physics.engine_cooling_conductance = 0.2
    ...
    assert debug["engine_cold_locked"]
    assert not debug["engine_running"]
    # A cold-locked engine burns nothing, so it can only drift towards ambient.
    assert debug["engine_temperature"] < config.physics.cold_start_temperature


def test_high_rpm_warms_engine_faster_than_idle() -> None:
    config = EnvConfig()
    config.physics.engine_cooling_conductance = 0.0
    config.physics.engine_cooling_airflow = 0.0
    config.physics.engine_heat_per_fuel = 730.0
    config.physics.engine_idle_heat = 2.2
    ...
    # Both engines idle-heat; burning fuel at high rpm must still warm faster.
    assert revving["engine_temperature"] > idle["engine_temperature"] + 1.0
