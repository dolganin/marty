...
def _score_trial(model_path: Path, config_path: Path, trial_seed: int, *,
    ...
    config_data.setdefault("env", {})["episodes_per_trial"] = 1_000_000
    with tempfile.TemporaryDirectory(prefix="mars_adaptation_") as temporary:
        pinned_config = Path(temporary) / "pinned_trial.yaml"
        pinned_config.write_text(yaml.safe_dump(config_data, sort_keys=False), encoding="utf-8")
        vec_env = MarsRoverSb3VecEnv(
            1, list(ACTION_MACROS), config_path=str(pinned_config), biome_split=1, seed=trial_seed,
        )
